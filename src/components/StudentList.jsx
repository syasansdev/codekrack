      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-4xl font-bold text-fg mb-2">Student Directory</h1>
              <p className="text-fg-muted">Manage student profiles and track coding platform activities</p>
            </div>
            <div className="flex gap-3">
              <motion.button
                onClick={() => refetch()}
                className={`px-6 py-3 bg-surface text-fg-muted border-2 border-edge-strong rounded-lg transition-all font-semibold flex items-center gap-2 hover:shadow-md ${isRefreshing ? 'border-blue-500' : 'hover:border-blue-500 hover:text-blue-600'}`}
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                disabled={isRefreshing}
              >
                <motion.svg 
                  className={`w-5 h-5 ${isRefreshing ? 'text-blue-600' : 'text-fg-subtle'}`} 
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
        ...
        <motion.div 
          className="bg-surface border-2 border-edge rounded-xl p-6 mb-8 shadow-sm hover:shadow-md transition-all"
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <motion.div whileHover={{ scale: 1.01 }} className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 pl-10 border-2 border-edge-strong rounded-lg focus:outline-none focus:border-blue-500 transition-colors"
            ...
            <div>
              <label htmlFor="department" className="block text-sm font-semibold text-fg-muted mb-2">
        ...
        <motion.div 
          className="bg-surface border-2 border-edge rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all"
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        >
          {loading ? (
            <div className="p-16 text-center">
              <motion.div 
                className="inline-block rounded-full h-16 w-16 border-4 border-edge border-t-blue-600 mb-4"
                animate={{ rotate: 360 }}
                transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
              />
              <p className="text-fg-muted font-semibold text-lg">Loading student data...</p>
            </div>
          ) : students.length === 0 ? (
            <div className="p-16 text-center">
              <h3 className="text-2xl font-bold text-fg mb-3">No Students Yet</h3>
              <p className="text-fg-muted mb-8">Start by adding your first student.</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="bg-surface-2 border-b-2 border-edge">
                    <th className="px-6 py-4 text-left text-xs font-bold text-fg-muted uppercase">Student</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-fg-muted uppercase">Registration</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-fg-muted uppercase">Academic</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-fg-muted uppercase">Platforms</th>
                    <th className="px-6 py-4 text-right text-xs font-bold text-fg-muted uppercase">Actions</th>
                  </tr>
                </thead>
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="flex items-center">
                              ...
                              <div className="ml-4">
                                <div className="text-base font-bold text-fg">{student.name || 'No Name'}</div>
                                <div className="text-sm text-fg-muted">{student.email || 'No Email'}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm font-semibold text-fg">{student.registerNumber || 'N/A'}</div>
                            <div className="text-sm text-fg-muted">Roll: {student.rollNumber || 'N/A'}</div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex gap-2 flex-wrap">
                              {Object.entries(student.platformUrls || {}).map(([platform, url]) => (
                                url && (
                                  <motion.a 
                                    ...
                                    className="p-2 bg-surface-2 rounded-lg hover:bg-surface-3 transition-all"
                              ...
                              {(!student.platformUrls || Object.values(student.platformUrls).every(url => !url)) && (
                                <span className="text-xs text-fg-subtle py-2">No profiles</span>
                              )}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex justify-end gap-2">
                              ...
                              {/* <motion.button
                                ...
                                className={`px-4 py-2 bg-surface border-2 rounded-lg transition-all font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1 ${
                                  scrapingStatus[student.id] === 'scraping' 
                                    ? 'border-blue-500 text-blue-600' 
                                    : 'border-edge-strong text-fg-muted hover:border-green-500 hover:text-green-600'
                                }`}
                      ...
                      >
                        <td colSpan="5" className="px-6 py-12 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-fg-muted mb-4 text-lg font-medium">No students match your search criteria</p>
                            <motion.button
                              onClick={() => {
                                setSearchTerm('');
                                setFilterDepartment('');
