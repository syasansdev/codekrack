...
const PlatformIcon = ({ platform }) => {
  const icons = {
    ...
    hackerrank: (
      <svg viewBox="0 0 24 24" fill="#00EA64" className="h-8 w-8 transition transform hover:scale-110 duration-300">
                      ...
                      {isLoading ? (
                        ...
                        <dl>
                          ...
                          {platform === 'hackerrank' && stats && (
                            <>
                              <StatItem label="Problems Solved" value={stats.problemsSolved} />
                              <StatItem label="Badges Earned" value={stats.badgesCount} />
                              {stats.badges && stats.badges.length > 0 && (
                                <div className="col-span-2 mt-3 pt-3 border-t border-edge">
                                  <p className="text-xs font-semibold text-fg-muted mb-2">Track Badges</p>
                                  <div className="flex flex-wrap gap-2">
                                    {stats.badges.map(b => (
                                      <span key={b.name} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        {b.name} ({b.stars}★)
                                      </span>
                                    ))}
                                  </div>
