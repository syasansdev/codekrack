  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-ink-900">Student Access</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Students', value: stats.total, cls: 'text-ink-900' },
          ...
          <div key={s.label} className="card-elite p-4">
            ...
            <div className="text-xs font-medium text-ink-500 mt-0.5">{s.label}</div>
          </div>
        ))}
      </div>
              ...
              {filtered.map((s) => (
                <tr key={s.id}>
                  <td>
                    ...
                    {s.rollNumber && <div className="text-xs text-ink-400">{s.rollNumber}</div>}
                  </td>
                  <td><StatusPill state={s.accessState} /></td>
                  <td className="text-sm text-ink-500">{timeAgo(s.invitedAt) || '—'}</td>
                  <td className="text-sm text-ink-500">{timeAgo(s.lastSignInAt) || 'Never'}</td>
      ...
      {isSuperAdmin && (
        <div className="card-elite overflow-hidden">
          <div className="p-4 border-b border-ink-100">
            <h2 className="font-display text-lg font-semibold text-ink-900">Institution admin logins</h2>
            ...
            <table className="data-table w-full">
              ...
              <tbody>
                {institutions.map((inst) => {
                  ...
                  return (
                    <tr key={inst.id}>
                      <td className="font-medium text-ink-900">{inst.name}</td>
                      <td className="text-sm text-ink-600">{inst.adminEmail || <span className="text-ink-400">no admin</span>}</td>
                        ...
                        {inst.adminEmail ? (
                          <div className="flex gap-2">
                            <input
                              ...
                              className="px-3 py-1.5 text-sm border border-ink-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none"
                            />
                            <button
